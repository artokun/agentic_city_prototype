...
fn infer_research_topic(
    agent_entity: Entity,
    agent_name: &AgentName,
    bounty_registry: &BountyTokenStore,
) -> String {
    let Some(bounty) = bounty_registry.tokens.values().find(|b| {
        b.claimed_by == Some(agent_entity) && b.state == crate::world::bounty::BountyState::Claimed
    }) else {
        return format!("research assignment for {}", agent_name.0);
    };

    extract_research_topic(&bounty.hidden_criteria)
        .or_else(|| extract_research_topic(&bounty.description))
        .unwrap_or_else(|| bounty.description.clone())
}

fn extract_research_topic(text: &str) -> Option<String> {
    let lower = text.to_ascii_lowercase();

    if let Some(start) = lower.find("research the ") {
        let topic_start = start + "research the ".len();
        let tail = &text[topic_start..];
        let end = tail.find('.').unwrap_or(tail.len());
        let topic = tail[..end].trim();
        if !topic.is_empty() {
            return Some(topic.to_string());
        }
    }

    if lower.contains("research paper") {
...
fn pick_service_for_need(
    ...
    for service in &relevant {
        if service.gold_cost > agent_gold {
            continue;
        }
        for (entity, entrance, name) in structures {
            if name != service.building_name {
                continue;
            }
...
pub fn execution_system(
    ...
    mut structures: Query<
        (
            ...
            &SpriteType,
            &mut Inventory,
            Option<&mut Staffable>,
        ),
        (With<StructureId>, Without<AgentName>),
    >,
) {
    let Some(mut bounty_registry) = boards_exec.iter_mut().next() else {
        return;
    };
    let board_info: Vec<(Entity, GridPos)> = boards.iter().map(|(e, ent, _)| (e, ent.0)).collect();
    let Some((board_entity, board_entrance)) = board_info.first().copied() else {
        return;
    };

    let structure_list: Vec<(Entity, GridPos, String)> = structures
        .iter()
        .map(|(e, ent, sprite, _, _)| (e, ent.0, sprite.0.clone()))
        .collect();
    ...
    ) in &mut agents
    {
        ...
        if *goal == AgentGoal::PerformingAction {
            ...
            let active_bounty = bounty_registry
                .tokens
                .values()
                .find(|b| {
                    b.claimed_by == Some(agent_entity)
        ...
        match current_goal {
            ...
            AgentGoal::Idle => {
                // No auto-handling. Agents make ALL their own decisions via AI.
                // If needs hit 0, the consequences play out (pass out, hospital, etc).
            }
                ...
                if !has_path {
                    ...
                    if at_entrance {
                        ...
                        if service == "work_shift" {
                            let building_name = structure_list
                                .iter()
                                .find(|(e, _, _)| *e == building)
                                .map(|(_, _, s)| s.clone())
                                .unwrap_or_default();
                            // Read ticks_per_gold from Staffable and mark building as staffed.
                            let ticks_per_gold = if let Ok((_, _, _, _, Some(mut staffable))) =
                                structures.get_mut(building)
                            {
                            ...
                            commands
                                .entity(agent_entity)
                                .insert(InsideBuilding(building));
                            action_log.log(
                                tick.0,
                            ...
                            *goal = AgentGoal::WorkingShift { building };
                            ...
                            let svc_building_name = structure_list
                                .iter()
                                .find(|(e, _, _)| *e == building)
                                .map(|(_, _, s)| s.clone())
                                .unwrap_or_default();
                                ...
                                if svc.action_name == "search_internet" {
                                    if inv.has(ItemType::GoldCoin, svc.gold_cost) {
                                        ...
                                        commands
                                            .entity(agent_entity)
                                            .insert(InsideBuilding(building));
                                        let topic = infer_research_topic(
                                            agent_entity,
                                            name,
                                            &bounty_registry,
                                        );
                                        commands
                                            .entity(agent_entity)
                                            .insert(super::gm::PendingResearch { topic });
                                        *goal = AgentGoal::PerformingAction;
                                    } else {
                        ...
                        if let Some((_, entrance, _)) =
                            structure_list.iter().find(|(e, _, _)| *e == building)
                        {
            ...
            AgentGoal::ReturningToBoard(bounty_id) => {
                ...
                if !has_path && at_pos(pos, &board_entrance) {
                    tracing::info!(
                        "[SUBMIT] {} submitting bounty {} for GM verification",
                        name.0,
                        bounty_id
                    );
                    ...
                    if let Some(bounty) = bounty_registry.get(bounty_id).cloned() {
                        if bounty.expired {
                            ...
                            thought.0 = format!(
                                "Recycled expired bounty (-{}g)",
                                crate::world::bounty::recycle_cost()
                            );
                            tracing::info!(
                                "{} recycled expired bounty (-{}g)",
                                name.0,
                            ...
                            thought.0 =
                                "Bounty submitted for Game Master review. Waiting for verdict..."
                                    .into();
                            if let Some(b) = bounty_registry.tokens.get_mut(&bounty_id) {
