...
pub fn serialize_world(
            ...
            |(
                ...
                if let Some(c) = cards {
                    for (contact_name, _) in &c.contacts {
                        ...
                        inv_slots.push(fb::InventorySlot::create(
                            &mut fbb,
                            &fb::InventorySlotArgs {
                                item_type: Some(item_str),
                                count: 1,
                            },
                        ));
                    }
                ...
                let mut named_item_labels = std::collections::HashSet::new();
                if let Some(contained) = contained {
                    for item_entity in &contained.items {
                        if let Ok((kind, item_name, token_info)) = item_entities.get(*item_entity) {
                            match kind.0 {
                                ItemType::Document => {
                                    if let Some(item_name) = item_name {
                                        let label = format!("doc:{}", item_name.0);
                                        named_item_labels.insert(label.clone());
                                        let item_str = fbb.create_string(&label);
                                        inv_slots.push(fb::InventorySlot::create(
                                            &mut fbb,
                                            &fb::InventorySlotArgs {
                                                item_type: Some(item_str),
                                ...
                                ItemType::BountyToken => {
                                    let label = token_info
                                        .map(|info| format!("bounty: {}", info.title))
                                        .or_else(|| item_name.map(|name| name.0.clone()))
                                        .unwrap_or_else(|| "bounty_token".to_string());
                                    named_item_labels.insert(label.clone());
                                    let item_str = fbb.create_string(&label);
                                    inv_slots.push(fb::InventorySlot::create(
                                        &mut fbb,
                ...
                // Add named documents from DocumentInventory as a legacy fallback.
                if let Some(docs) = doc_inv {
                    for title in docs.documents.keys() {
                        let label = format!("doc:{}", title);
                        if named_item_labels.insert(label.clone()) {
                            let item_str = fbb.create_string(&label);
                            inv_slots.push(fb::InventorySlot::create(
                                &mut fbb,
                                &fb::InventorySlotArgs {
                ...
                if !named_item_labels
                    .iter()
                    .any(|label| label.starts_with("bounty: "))
                {
                    if let Some(bounty) = bounty_registry.tokens.values().find(|b| {
                        b.claimed_by == Some(entity)
                            && b.state == crate::world::bounty::BountyState::Claimed
                    }) {
                        let item_str =
                            fbb.create_string(&format!("bounty: {}", bounty.description));
                        inv_slots.push(fb::InventorySlot::create(
                            &mut fbb,
                            &fb::InventorySlotArgs {
                                item_type: Some(item_str),
                ...
                // Relationships.
                let rel_offsets: Vec<_> = rels
                    .known
                    .values()
                    .map(|mem| {
                        let name = fbb.create_string(&mem.name);
                        let goal = fbb.create_string(&mem.last_known_goal);
                        fb::RelationshipSnapshot::create(
                            &mut fbb,
                            &fb::RelationshipSnapshotArgs {
                ...
                let chat_offsets: Vec<_> = convo_log
                    .map(|log| {
                        log.messages
                            .iter()
                            .map(|m| {
                                ...
                                fb::ChatMessageSnapshot::create(
                                    &mut fbb,
                                    &fb::ChatMessageSnapshotArgs {
                ...
                let vis_offsets: Vec<_> = vision
                    ...
                    .map(|v| {
                        ...
                        fb::VisibleEntitySnapshot::create(
                            &mut fbb,
                            &fb::VisibleEntitySnapshotArgs {
                                name: Some(name),
                                pos: Some(&pos),
                                kind: Some(kind),
                                distance: v.distance,
                ...
                // Tracking.
                let track_offsets: Vec<_> = tracking
                    .targets
                    .values()
                    .map(|t| {
                        let name = fbb.create_string(&t.name);
                        let pos = fb::Vec2i::new(t.last_known_pos.x, t.last_known_pos.y);
                        fb::TrackedEntitySnapshot::create(
                            &mut fbb,
                            &fb::TrackedEntitySnapshotArgs {
                                name: Some(name),
                                pos: Some(&pos),
                                in_range: t.in_range,
                            },
                ...
                fb::AgentSnapshot::create(
                    &mut fbb,
                    &fb::AgentSnapshotArgs {
                        id: Some(id_str),
                        name: Some(name_str),
                        pos: Some(&fb_pos),
                        animation: fb_anim,
    ...
    let struct_offsets: Vec<_> = structures
        .iter()
        .map(|(id, pos, sprite, interactable, inv, docs, contained, _)| {
            let id_str = fbb.create_string(&id.0.to_string());
            ...
            if let Some(contained) = contained {
                for item_entity in &contained.items {
                    if let Ok((kind, item_name, _token_info)) = item_entities.get(*item_entity) {
                        if kind.0 == ItemType::Document {
                            if let Some(item_name) = item_name {
                                let label = format!("doc:{}", item_name.0);
                                named_item_labels.insert(label.clone());
                                let item_str = fbb.create_string(&label);
            ...
            let inv_vec = fbb.create_vector(&inv_slots);
            fb::StructureSnapshot::create(
                &mut fbb,
                &fb::StructureSnapshotArgs {
                    id: Some(id_str),
                    pos: Some(&fb_pos),
                    sprite_type: Some(sprite_str),
    ...
    let bounty_offsets: Vec<_> = bounty_registry
        .tokens
        .values()
        ...
        .map(|b| {
            ...
            let claimed_str = b
                .claimed_by
                .and_then(|e| agent_names.get(e).ok().map(|n| fbb.create_string(&n.0)));
            ...
            fb::BountySnapshot::create(
                &mut fbb,
                &fb::BountySnapshotArgs {
                    id: Some(id_str),
                    description: Some(desc_str),
                    target_pos: Some(&fb_pos),
                    reward_gold: b.reward_gold,
                    state: fb_state,
                    claimed_by: claimed_str,
                },
            )
        })
        .collect();
    ...
    let board_queue = board_queues.iter().next().map(|queue| {
        let interacting_str = queue
            .interacting
            .and_then(|e| agent_names.get(e).ok().map(|n| fbb.create_string(&n.0)));
        let waiting_strs: Vec<_> = queue
            .waiting
            .iter()
            .filter_map(|e| agent_names.get(*e).ok().map(|n| fbb.create_string(&n.0)))
            .collect();
        let waiting_vec = fbb.create_vector(&waiting_strs);
        fb::BoardQueueSnapshot::create(
            &mut fbb,
            &fb::BoardQueueSnapshotArgs {
                interacting: interacting_str,
                waiting: Some(waiting_vec),
            },
        )
    });

    // Event log.
    let log_offsets: Vec<_> = event_log
        .entries
        .iter()
        .map(|e| {
            ...
            fb::LogEntrySnapshot::create(
                &mut fbb,
                &fb::LogEntrySnapshotArgs {
                    tick: e.tick,
                    agent: Some(agent),
                    kind: Some(kind),
                    text: Some(text),
    ...
    let world = fb::WorldSnapshot::create(
        &mut fbb,
        &fb::WorldSnapshotArgs {
            tick: tick.0,
            agents: Some(agents_vec),
            structures: Some(structures_vec),
            bounties: Some(bounties_vec),
            board_queue,
            event_log: Some(log_vec),
        },
    );
...
fn serialize_inventory<'a, A: flatbuffers::Allocator + 'a>(
    ...
    inv.items
        ...
        .filter(|(item, count)| {
            **item != ItemType::Document && **item != ItemType::BountyToken && **count > 0
        })
        .map(|(item, count)| {
            let item_str = fbb.create_string(&item.to_string());
            fb::InventorySlot::create(
                fbb,
                &fb::InventorySlotArgs {
                    item_type: Some(item_str),
                    count: *count,
